...
def inicio():
    ...
    enemy0 = Inimigo(player.rect, player, 966,322, False,spritesheet_inimigo_arco, 10, 750, 50)
    enemy1 = Inimigo(player.rect, player, 950,754, False,spritesheet_inimigo_arco1, 13, 500, 20)
    enemy2 = Inimigo(player.rect, player, 1470,102, True,spritesheet_inimigo_arco2, 8, 650, 30)
    enemy3 = Inimigo(player.rect, player, 950,266, False,spritesheet_inimigo_arco3, 9, 600, 40)
    ...
    pocao1 = Item('consumivel', 'Poção', {'vida': 10},False,player)

    bau_saida = Bau(screen,1000,100,[pocao,pocao1])
    ...
    while menu_opcoes.rodando:
        ...
        if len(inimigos) == 0 and player.rect.y < 64:
            salvar_game()
            screen.fill((0, 0, 0))
            fundo_loading = pygame.image.load(os.path.join(assets,'tela_loading_ligeiro.png')).convert_alpha()
